...
const COLUMNS = ["Specialist Visit", "Generic drug", "Primary care visit to treat an injury or illness", "Emergency room services", "Urgent care centers or facilities", "Mental behavioral health inpatient services", "Mental behavioral health outpatient services"]
            ...
            <CardContent sx={{ display: "flex", justifyContent: "space-evenly" }}>
                <Box sx={{ width: "50%" }}><CheckBoxIcon /><Typography>Has:</Typography>
                    {hospital.row.map((benefit: any, index: number) => {
                        ...
                        if (benefit)
                            return <p key={index}>
                                <Typography sx={{ textAlign: "center" }}>{COLUMNS[index - 5]}</Typography>
                            </p>
                    })}
                </Box>
...
const MapComponent = ({ lat, long, hospitals, radiusInKm }: any) => {
    ...
    const render = useMemo(() => {
        return hospitals.map((hospital: any, index: number) => {
            if (getDistance(lat, long, hospital.latitude, hospital.longitude) > radiusInKm) return null;
            return (
